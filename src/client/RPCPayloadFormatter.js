...
import * as albumArt from '../../config/art.json' with {type: 'json'};
...
export default (status) => {
  ...
  const output = {
    details: (meta.title.length == 1 ? `${meta.title} ` : meta.title) || (meta.filename.length == 1 ? `${meta.filename} ` : meta.filename),
    type: 2,
